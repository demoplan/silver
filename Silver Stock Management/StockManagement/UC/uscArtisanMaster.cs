...
{
    public partial class uscArtisanMaster : UserControl
    {
        DAL.Model.SilverEntities entities;
        private int selectedID = 0;

        public uscArtisanMaster()
        {
            InitializeComponent();
        ...
        private void LoadGridData()
        {
            using (entities = new DAL.Model.SilverEntities())
            {
                var datasource = from row in entities.CustomerMs where row.CustType.Equals("A") select row;
